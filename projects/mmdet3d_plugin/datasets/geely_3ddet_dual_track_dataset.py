...
class Geely3DDetTrackDatasetDualBox(Dataset):
    DefaultAttribute = {
        "car": "vehicle.parked",
    ...
    def get_data_info(self, index):
        ...
        if self.modality["use_camera"]:
            ...
            for cam_type, cam_info in info["cams"].items():
                if "_A_" in cam_type:
                    continue
                image_key.append(cam_type)
                image_paths.append(cam_info["data_path"])
                # obtain lidar to image transformation matrix
                if "ego2cam" in cam_info:
                    lidar2cam = cam_info['ego2cam']
                elif "lidar2cam" in cam_info:
                    ...
                    ego2sensor_r = np.linalg.inv(sensor2lidar_rotation)
                    ego2sensor_t = sensor2lidar_translation @ ego2sensor_r.T
    ...
    def get_ann_info(self, index):
        ...
        mask = np.ones(info["gt_boxes"].shape[0]).astype(np.bool)
        gt_bboxes_3d = info["gt_boxes"][mask]
        # gt_bboxes_3d[:, -1] -= np.pi / 2
        ...
        gt_bboxes_3d_img = info["gt_boxes_img"]
        # gt_bboxes_3d[:, -1] -= np.pi / 2
        gt_visibility = info['visibility']
        gt_names_3d_img = info["gt_names_img"]
        gt_labels_3d_img = []
        for cat in gt_names_3d_img:
            if cat in self.CLASSES:
                gt_labels_3d_img.append(self.CLASSES.index(cat))
            else:
                gt_labels_3d_img.append(-1)
        gt_labels_3d_img = np.array(gt_labels_3d_img)

        if self.with_velocity:
            gt_velocity_img = info["gt_velocity_img"]
            nan_mask = np.isnan(gt_velocity_img[:, 0])
            gt_velocity_img[nan_mask] = [0.0, 0.0]
            gt_bboxes_3d_img = np.concatenate([gt_bboxes_3d_img, gt_velocity_img], axis=-1)

        anns_results = dict(
            gt_bboxes_3d=gt_bboxes_3d,
            gt_labels_3d=gt_labels_3d,
            gt_names=gt_names_3d,
            gt_bboxes_3d_img=gt_bboxes_3d_img,
            gt_labels_3d_img=gt_labels_3d_img,
            gt_names_img=gt_names_3d_img,
        ...
        if "instance_inds" in info:
            ...
            instance_inds_img  = np.array(info["instance_inds_img"], dtype=np.int)
            anns_results["instance_inds_img"] = instance_inds_img 
        return anns_results
    ...
    def save_pred(self, pts,imgs_pure,pred_box,lidar2img,gt_bboxes_3d,gt_labels_3d,save_dir,every_frame=False):
        if every_frame and not hasattr(self,"save_count"):
            self.tmp_trackid=[]
            self.save_count=1
        imgs = []
        mask_box = pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
        pred_box['img_bbox']['boxes_3d'] = pred_box['img_bbox']['boxes_3d'][
                pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
            ]
        pred_box['img_bbox']["instance_ids"] = pred_box['img_bbox']["instance_ids"][
                pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
            ]
        pred_box['img_bbox']['cls_scores']= pred_box['img_bbox']["cls_scores"][
                pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
            ]
        pred_box['img_bbox']['labels_3d']= pred_box['img_bbox']['labels_3d'][
                pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
            ]

        if "vis_ids" in pred_box['img_bbox']:
            pred_box['img_bbox']['vis_ids'] = pred_box['img_bbox']['vis_ids'][
                pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
            ]
            pred_box['img_bbox']['vis_scores'] = pred_box['img_bbox']['vis_scores'][
                pred_box['img_bbox']["scores_3d"] > self.vis_score_threshold
            ]
            vis_info=pred_box['img_bbox']['vis_ids']
        else:
            vis_info=None
        pred_box['img_bbox']['scores_3d'] = pred_box['img_bbox']['scores_3d'][
                pred_box['img_bbox']["scores_3d"] > 0.3
            ]
        # if "instance_ids" in pred_box['img_bbox'] and self.tracking:
        #     color = []
        #     for id in  pred_box['img_bbox']["instance_ids"].cpu().numpy().tolist():
        #         color.append(
        #             self.ID_COLOR_MAP[int(id % len(self.ID_COLOR_MAP))]
        #         )
        if "labels_3d" in  pred_box['img_bbox']:
            color = []
        ...
        pred_bboxes_3d_img =  pred_box['img_bbox_by_img']['boxes_3d'][ pred_box['img_bbox_by_img']['scores_3d']>self.vis_score_threshold]
        ...
        bev = draw_lidar_bbox3d_points_on_bev(
                pts,
                pred_bboxes_3d,
                bev_size=img.shape[0] * 3*3,
                color=color,
                bev_range=300,
                track_id = pred_box['img_bbox']["instance_ids"] ,
        ...
        self.tmp_trackid = list(pred_box['img_bbox']["instance_ids"].cpu().numpy())
